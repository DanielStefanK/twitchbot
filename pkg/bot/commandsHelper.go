...
type commandHandler func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot)
...
var devHandler = func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
...
var allCommands = [15]*Command{
	...
	&Command{Cmd: "time", DBNum: calNumber(1), Help: "Get the time in the given country/city", Handler: func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
		//TODO: implement
		reply(fmt.Sprintf("not implemented yet"))
	}},
	...
	&Command{Cmd: "pat", DBNum: calNumber(5), Help: "Pat a user", Handler: func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
		...
		users, err := bot.client.Userlist(msg.Channel)
	...
	&Command{Cmd: "soulmate", DBNum: calNumber(6), Help: "Find a soulmate in the chat", Handler: func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
		users, err := bot.client.Userlist(msg.Channel)
	...
	&Command{Cmd: "push", DBNum: calNumber(12), Help: "push someone off a cliff", Handler: func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
		params := getParams(msg.Message)
		users, err := bot.client.Userlist(msg.Channel)
		...
		if len(params) == 0 || len(params) > 1 {
			reply("you can push one other chatter at a time")
			return
		}

		if !contains(users, params[0]) {
			//FIXME: does this work
			reply(fmt.Sprintf("the user %s does not appear to be in the chat right now. Maybe push him later", params[0]))
			return
		}

		reply(fmt.Sprintf("%s is pushing %s off a cliff!", msg.User.DisplayName, params[0]))
	}},
	&Command{Cmd: "hug", DBNum: calNumber(13), Help: "hug someone in chat", Handler: func(db *gorm.DB, msg twitch.PrivateMessage, reply replyFunc, bot *Bot) {
		params := getParams(msg.Message)
		users, err := bot.client.Userlist(msg.Channel)
		...
		reply(fmt.Sprintf("%s is giving %s a hug!", msg.User.DisplayName, params[0]))
	}},
}
